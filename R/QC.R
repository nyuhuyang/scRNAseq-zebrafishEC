...
if(length(missing_data)>0){
        ...
        for(missing_dat in missing_data){
                ...
                list_gz_files = grep("\\.gz$", list_files, value =T)
                lapply(paste(new.folder, list_gz_files,sep = "/"), gunzip)
                file.rename(paste(new.folder, "features.tsv",sep = "/"),
                            paste(new.folder, "genes.tsv",sep = "/"))
        }
}
...
#object_sce <- Convert(from = object, to = "sce")
#location <- Drerio_gene[match(rownames(object_sce),Drerio_gene$gene_name),"seq_name"]
#names(location) = rownames(object_sce)
#table(location)
#rowData(object_sce)$CHR <- location
